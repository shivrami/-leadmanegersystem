...
const OpenLead = () => {
  ...
  const handleFollowupSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:8080/api/followups/lead/${currentLead.id}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
      ...
      await fetch(
        `http://localhost:8080/api/leads/${currentLead.id}/followup`,
        {
          method: 'PUT',
      ...
      if (response.ok) {
        ...
        updateFollowUpDate();
        navigate('/counselor/todayfollowups');
      } else {
        throw new Error('Followup submission failed');
      }
    } catch (error) {
  ...
  const updateFollowUpDate = async () => {
    const today = new Date().toISOString().split('T')[0];
    try {
      const response = await fetch(`http://localhost:8080/api/leads/${currentLead.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json'
        },
          ...
          <div className="modal-dialog">
            <form className="modal-content" onSubmit={handleFollowupSubmit}>
              ...
              <div className="modal-footer">
                ...
                <button type="submit" className="btn btn-primary">Submit Follow-up</button>
              </div>
            </form>
          </div>
        </div>
